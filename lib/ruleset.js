...
Ruleset.prototype.tryRead = function(path, root, auth, now) {
  ...
  for (var i = 0; i < rules.length; i++) {
    ...
    state.data = root.child(ruleDef.path);
  ...
  if (result.allowed) {
    ...
    result.shortInfo += 'No .read rule allowed the operation.\n';
    result.shortInfo += 'Read was denied.';
  }
...
Ruleset.prototype.tryPatch = function(path, root, newData, auth, skipWrite, skipValidate, skipOnNoValue, now) {
  ...
  if (pathResults.length === 0) {
    ...
    result = pathResults.reduce(function(result, pathResult) {
      ...
      result.shortInfo += pathResult.shortInfo;
...
Ruleset.prototype._tryWrite = function(path, root, newDataRoot, result, skipWrite, skipValidate, skipOnNoValue) {
  ...
  (function traverse(pathParts, remainingPath, rules, wildchildren) {
    ...
    if (!skipWrite && !result.writePermitted && rules.hasOwnProperty('.write')) {
      ...
      rule = rules['.write'];
      var pathString = '/' + currentPath + ':.write: "' + rule + '"\n';
      result.info += pathString;
      ...
      try {
        ...
        if (rule.evaluate(state) === true) {
          ...
          result.info += '    => false\n';
          writeInfo += pathString + '    => false\n';
        }

      } catch(e) {
        result.writePermitted = false;
        result.info += e.message + '\n';
        writeInfo += pathString + e.message + '\n';
      }
    ...
    if (
      ...
      rule = rules['.validate'];
      var validatePath = '/' + pathParts.join('/') + ':.validate: "' + rule.toString() + '"\n';
      result.info += validatePath;
      ...
      try {
        ...
        if (rule.evaluate(state, skipOnNoValue) === true) {
          ...
          result.info += '    => false\n';
          validateInfo += validatePath + '    => false\n';
        }

      } catch(e) {
        result.validated = false;
        result.info += e.message + '\n';
        validateInfo += validatePath + e.message + '\n';
      }
  ...
  if (!result.writePermitted) {
    result.shortInfo += writeInfo;
  }
  if (!result.validated) {
    result.shortInfo += validateInfo;
  }

  if (!result.writePermitted) {
    result.info += 'No .write rule allowed the operation.\n';
    result.info += 'Write was denied.\n';
    result.shortInfo += 'No .write rule allowed the operation.\n';
    result.shortInfo += 'Write was denied.\n';
  } else if (!result.validated) {
    result.info += 'Validation failed.\n';
    result.info += 'Write was denied.\n';
    result.shortInfo += 'Validation failed.\n';
    result.shortInfo += 'Write was denied.\n';
